...
| # | Баг | Статус |
|---|-----|--------|
| B01 | Кредиты только в localStorage | ✅ Закрыт — /api/pdf валидирует план на сервере; decrement-premium endpoint |
...
| B19 | Счётчик «3 КП» в SSR | ✅ Закрыт — `creditsMounted` флаг, счётчик скрыт до гидратации |
...
### Сессия 2026-03-25 — Четвёртый цикл исправлений

| # | Баг | Что сделано | Файл | Статус |
|---|-----|-------------|------|--------|
| B01 | Кредиты только в localStorage | 1) `/api/pdf` теперь проверяет сессию + серверный план перед генерацией premium PDF (403 для гостей и free). 2) Новый эндпоинт `POST /api/user/credits/decrement-premium` — списывает VIP/Modern кредиты в KV. 3) `result/page.tsx`: исправлен мёртвый код credits-sync (перенесён в отдельный useEffect); `handleTemplateSelect` вызывает сервер вместо localStorage. | `app/api/pdf/route.ts`, `app/api/user/credits/decrement-premium/route.ts`, `app/result/page.tsx` | ✅ |
| B19 | Счётчик «3 КП» до гидратации | Добавлен `creditsMounted` флаг — счётчик не рендерится до hydration | `app/page.tsx` | ✅ |
| B12 | /ref только JS | Переписан как серверный компонент с `cookies()` + `redirect()` | `app/ref/page.tsx` | ✅ |

**npm run build: ✅ 0 ошибок**

---

### Сессия 2026-03-26 — Пятый цикл исправлений

| # | Что сделано | Файл | Статус |
|---|-------------|------|--------|
| Resend домен | Верифицирован домен `kp-za-30.ru` в Resend (DKIM + SPF). Добавлена переменная `RESEND_FROM_EMAIL=noreply@kp-za-30.ru` в Vercel. Письма теперь приходят на любой email. | Vercel env + Resend DNS | ✅ |
| ResultPaywall | `/result` страница использовала `/api/payment/mock` напрямую — был заблокирован в prod. Переписан на `/api/payment/yookassa`. | `app/result/page.tsx` | ✅ |
| YooKassa 500 → реальная ошибка | `yookassa/route.ts` возвращал `{mock:true}` при ошибке → фронт пытался запустить заблокированный mock → "Не удалось провести оплату". Теперь возвращает `{error: message}` с реальной ошибкой. | `app/api/payment/yookassa/route.ts` | ✅ |
| YooKassa 400 Receipt | **Критично:** ЮКасса требует фискальный чек (54-ФЗ) с каждым платежом. Ошибка: `"Receipt is missing or illegal"`. Добавлен `receipt{customer{email}, items[{description, quantity, amount, vat_code:1, payment_mode, payment_subject}]}` в тело запроса `createPayment`. Email берётся из сессии пользователя. | `lib/yookassa.ts`, `app/api/payment/yookassa/route.ts` | ✅ |

**Коммиты:** `5e43f80`, `32a47c3`, `1744082`
