...
- [x] **[F01] Тариф «Разово» 199 ₽** — PaywallModal с двумя вариантами (разово 199₽ / подписка 990₽/мес), mock API `/api/payment/mock`, лимит 3 бесплатных КП в localStorage (`kp_free_count`), платные кредиты (`kp_paid_credits`), динамический счётчик в хедере на главной и в дашборде. Инициализация счётчика учитывает уже существующую историю КП. Подсказка про смену статуса в дашборде.
